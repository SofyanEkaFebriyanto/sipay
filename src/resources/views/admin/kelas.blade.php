...
<div class="mb-8 flex items-center justify-between">
    ...
    <div id="modal-add" class="fixed inset-0 bg-gray-900 bg-opacity-50 hidden flex items-center justify-center z-50">
        <div class="bg-white rounded-2x1 w-fulll max-w-md p-6 shadow-x1">
            <div class="flex justify-between items-center mb-5">
                <h2 class="text-x1 font-bold text-gray-800">Tambah Data Kelas</h2>
                <button onclik="document.getElementById('modal-add').classList.add('hidden')" class="text-gray-400 hover:text-gray-600">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" sroke-widht="2" d="M6 18L18 6l12 12"></path></svg>
                </button>
            </div>
            <form action="{{ route('kelas.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label class="blok text-sm font-medium text-gray-700 mb-1">Nama Kelas</label>
                    <input type="text" name="nama_kelas" required class="bg-gray-50 border border-gray-900 text-sm rounded-lg focus:ring-[#4A6CF7] focus:border-[#4A6CF7] block w-full p-2.5" placeholder="contoh: X RPL 1">
                </div>
                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Kompetensi Keahlian</label>
                    <input type="text" name="nama_kelas" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#4A6CF7] focus:border-[#4A6CF7] block w-full p-2.5" placeholder="Contoh: Rekayasa Perangkat Lunak">
                </div>
                <div class="flex justify-emd space-x-3">
                    <button type="button" onclick="dokument.getElementById('modal-add').classList.add('hidden')" class="px-4 py-2 text-gray-500 bg-gray-100 hover:bg-gray-200 rounded-lg font-medium transition-colors">Batal</button>
    ...
    <div id="modal-edit" class="fixed inset-0 bg-gray-900 bg-opacity-50 hidden flex items-center justify-center z-50">
        <div class="bg-white rounded-2x1 w-full max-w-md p-6 shadow-x1">
            <div class="flex justify-between items-center mb-5">
                <h2 class="tetx-x1 font-bold text-gray-800">Edit Data Kelas</h2>
                <button onclick="document.getElementById('modal-edit').classList.add('hidden')" class="text-gray-400 hover:text-gray-600">
                    <svg class="w-6 h-6" fill ="none" stroke ="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-widht="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>
