<x-app-layout>
            ...
            <tbody class="text-gray-700 divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="p-4 flex justify-center space-x-2">
                        <button onclick="openEditModal('{{ $kelas->id_kelas }}', '{{ addslashes($kelas->nama_kelas) }}', '{{ addslashes($kelas->kompetensi_keahlian) }}')" class="w-8 h-8 rounded-full bg-orange-100 text-orange-500 flex items-center justify-center hover:bg-orange-200 transition-colors" title="Edit">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                        </button>
                        <form action="{{ route('kelas.destroy', $kelas->id_kelas) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus data kelas ini? Jika ada siswa di kelas ini, penghapusan bisa dibatalkan oleh database constraints.');">
                            @csrf
                            @method('DELETE')
    ...
    <!-- Script untuk menangani logika modal edit -->
    <script>
