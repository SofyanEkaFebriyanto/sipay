...
<head>
    ...
    <title>@yield('title', 'SIPAY')</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>
<body class="bg-[#F3F4F6] text-gray-800 font-sans flex h-screen overflow-hidden">

    @php
        $user = Auth::guard('petugas')->user() ?? Auth::guard('siswa')->user();
        $role = '';
        $name = '';
        $dashboardRoute = '#';

        if (Auth::guard('petugas')->check()) {
            $role = $user->level; // 'admin' atau 'petugas'
            $name = $user->nama_petugas;
            $dashboardRoute = ($role == 'admin') ? route('admin.dashboard') : route('petugas.dashboard');
    ...
    <!-- Sidebar: Bagian navigasi samping -->
    <aside class="w-64 bg-[#4A6CF7] text-white flex flex-col hidden md:flex shrink-0 shadow-lg z-10">
        ...
        <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
            ...
            <!-- Menu Khusus Admin (Manajemen Data Master) -->
            @if($role === 'admin')
        ...
        <header class="h-20 bg-white flex items-center px-8 border-b border-gray-200 z-0">
            ...
            <div class="flex items-center space-x-3">
                ...
                <div class="text-sm hidden lg:block text-left">
                    <p class="font-bold text-gray-800">{{ $name ?? 'User' }}</p>
                    <p class="text-xs text-gray-500 font-medium capitalize">{{ $role ?? 'Guest' }}</p>
                </div>
            </div>
        </header>

        <!-- Area Main: Tempat konten halaman ditampilkan -->
