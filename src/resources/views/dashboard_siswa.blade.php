...
@section('content')
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-800 tracking-tight">Selamat Datang, {{ $siswa->nama }}</h1>
        <p class="text-gray-500 mt-1 font-medium">Berikut adalah histori pembayaran SPP Anda.</p>
    </div>

    <!-- Kartu Informasi Siswa: Menampilkan profil singkat dan total pembayaran -->
    <div class="bg-gradient-to-r from-[#4A6CF7] to-[#3451b2] rounded-2xl p-8 shadow-md text-white mb-8 relative overflow-hidden">
        ...
        <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
                ...
                <h2 class="text-3xl font-bold mb-2">{{ $siswa->nama }}</h2>
                <div class="flex items-center space-x-4 text-sm font-medium">
                    <span class="bg-white/20 px-3 py-1 rounded-full">NISN: {{ $siswa->nisn }}</span>
                    <span class="bg-white/20 px-3 py-1 rounded-full">NIS: {{ $siswa->nis }}</span>
                </div>
            </div>
    ...
    <!-- Area Tabel Histori: Daftar lengkap riwayat pembayaran siswa -->
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="text-gray-700 divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="p-4 text-gray-500">{{ $bayar->petugas->nama_petugas ?? 'Admin' }}</td>
                </tr>
                @empty
                <tr>
